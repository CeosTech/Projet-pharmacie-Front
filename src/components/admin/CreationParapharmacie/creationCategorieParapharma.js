...
const CreationCategorieParapharma = (props) => {
    /* categorie */
    ...
    const fetchData = async () => {
        sendrequest(
            "get",
            "parapharmacie/categorie/?ordering=id",
            setList,
            setLoad
        );
    }
    ...
    async function sendChanges() {
        for (let i = 0; i < list.length; i++) {
            await axios.put(URL + "parapharmacie/categorie/" + list[i].id + '/',
                {
                    id: list[i].id,
                    nom: list[i].nom,
                }
    ...
    async function addCategory() {
        if (!check(list, category)) { // Check if the new category doesn't already exist
            await axios.post(URL + "parapharmacie/categorie/",
                {
                    nom: category,
                }
            )
    ...
    async function deleteValue(index) {
        await axios.delete(URL + "parapharmacie/categorie/" + index + '/'
        )
        let tmp = [...list];
        let i = tmp.findIndex((element) => element.id == index);
        if (i > -1) {
...
export default CreationCategorieParapharma;